...
- **문제 상황1:** 민감 개인 정보 자동 메일 발송 서비스
	- 수십개의 조합 별로 LP(조합원), 수십명에게 각각 투자확인서 PDF를 첨부하여 메일을 발송해야 했음. 현재 투자확인서는 하나의 통합된 파일로 존재하는 상황임. 메일 발송시 제목, 본문, 파일, 참조, 숨은 참조 등을 통제해야 하는 상황. 

	  
- 행동 1 : 발송계획보고서 작성까지 위임. 
	![[Screenshot 2026-02-22 at 7.42.05 AM.png]]

- **문제상황2 :** 정부 공고 모니터링 서비스
	- 60개의 정부 사이트의 공고를 모니터링할 수 있는 자동화 시스템을 구축해야 하는 상황. 엑셀 위의 공고를 쌓아주기로 했음. 어떻게 내개 해당 사이트의 크롤링이 적절하게 이루어지고 있다고 진실성을 담보할 수 있을까? 어떻게 테스트를 설계해야 할까? 어떻게 개발자가 아닌, 비개발자 이용자들이 이 사이트를 믿게 할 수 있을까?

- **행동2 :** 크롤링 단 -> csv 결과를 보고 cc에게 검수하라고 지시. 모니터링 실행 단, 헬스 체크 
	- ![[Screenshot 2026-02-22 at 7.44.07 AM.png]]
	![[Screenshot 2026-02-22 at 7.30.22 AM.png]]

- **문제상황3 :** 그래비티벤처스 DX
	- 사내 자체 어드민 웹 및 LP 시스템 구축 과정에서, 어떻게 해당 기능을 구현하게 해야할까. 그리고 어떻게 해당 기능이 잘 작동함을 확인할 수 있을까? 데이터가 CRUD 작동이 정상적으로 이루어지고 있는가?  프론트, 백, DB의 각 층위에서 보안 리스크가 잘 통제되고 있는가?
	- ![[Screenshot 2026-02-22 at 7.40.35 AM.png]]
...
![[Screenshot 2026-02-22 at 7.57.56 AM.png]]
